...
trait ProfileValidation
{
    ...
    public function a_new_password_field_should_be_follow_the_password_rules()
    {
        ...
        $res->assertUnprocessable()
            ->assertJsonPath('errors.new_password.0', 'The new password field must be at least 8 characters.')
            ->assertJsonPath('errors.new_password.1', 'The new password field must contain at least one symbol.')
            ->assertJsonPath('errors.new_password.2', 'The new password field must contain at least one number.');
    }
    ...
    public function a_new_password_and_current_password_must_be_different()
    {
        ...
        $res->assertUnprocessable()
            ->assertJsonPath('errors.new_password.0', 'The new password field and current password must be different.');
    }
}
